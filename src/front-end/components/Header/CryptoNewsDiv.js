...
export default function CryptoNewsDiv({data, value}) {
    ...
    if (data === undefined || data[value] === '' ){
        ...
        out = symbol + ": $" + Math.round((+price + Number.EPSILON)*100)/100 + " USD";
    }

    return (<div className={'crypto-news-component ' + colour}>
        {Math.round((+change_24hr + Number.EPSILON)*100)/100}
        <FontAwesomeIcon icon={change_24hr==='' ? faSpinner : change_24hr>=0 ? faSortAsc : faSortDesc} size="2x" />
